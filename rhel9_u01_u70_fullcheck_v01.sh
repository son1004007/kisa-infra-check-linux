...
# 결과:
#  - ./rhel9_unix_check_<TS>/rhel9_unix_check_<TS>.json
#  - ./rhel9_unix_check_<TS>/rhel9_unix_check_<TS>.log
#
...
JSON_TMP="${OUT_DIR}/results.tmp.json"
JSON_OUT="${OUT_DIR}/rhel9_unix_check_${TS}.json"
LOG_OUT="${OUT_DIR}/rhel9_unix_check_${TS}.log"
mkdir -p "${OUT_DIR}"
: > "${JSON_TMP}" ; echo "[" >> "${JSON_TMP}"
: > "${LOG_OUT}"

# helper: emit json item
emit() {
  local code="$1"; shift
  local title="$1"; shift
  local status="$1"; shift
  local detail="$*"
  # escape JSON properly using python
  local jtitle jdetail
  jtitle=$(python3 - <<PY
import json,sys
...
PY
  ...
  printf '  {"code":"%s","title":%s,"status":"%s","detail":%s},\n' "$code" "$jtitle" "$status" "$jdetail" >> "${JSON_TMP}"
  printf '%s | %s | %s\n' "$code" "$title" "$status" >> "${LOG_OUT}"
  printf '%s\n\n' "$detail" >> "${LOG_OUT}"
}
...
########## finish JSON: close array and tidy
...
with open(tmp,'r',encoding='utf-8') as f:
    s=f.read()
# remove last comma before newline
s=s.rstrip()
if s.endswith(','):
    s=s[:-1]
s=s + "\n]\n"
# attempt to pretty print to ensure valid JSON
try:
    arr=json.loads(s[s.index('['):])
...
echo "Finished. JSON: ${JSON_OUT}" | tee -a "${LOG_OUT}"
echo "Summary log: ${LOG_OUT}" | tee -a "${LOG_OUT}"
